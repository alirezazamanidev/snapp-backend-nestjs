...
import {
  ICheckDriverProfileRequest,
  IGetRoleRequest,
} from '@app/common/interfaces/user-grpc.interface';
...
export class UserService {
  ...
  async updateUserRole(
    ...
    // Invalidate all session caches for this user since role changed
    const sessions = await this.sessionRepository.find({
      where: { userId: sessionData.userId },
    });

    for (const userSession of sessions) {
      await this.redisClient.del(`session:${userSession.id}`);
    }
